...
derivative tc27A
{
    ...
    {
         prohibit_references_to :tc1:linear, :tc1:abs18;
    }  
    section_setup :tc0:linear
    {
        // prohibit_references_to :vtc:linear, :vtc:abs18;
    }
    ...
    /*	 group PSPR ( run_addr = mem:mpe:pflash0, nocopy, attributes=rx )
	    ...
	    //group  (ordered,run_addr = mem:dsram0, attributes=rx )
		//{
		 group my_core0_func(run_addr=mem:psram0[1k],ordered )
		 {
			select ".text.private0.*";
		 }
		 group my_core0_data(run_addr=0x70002000,ordered )
		 {
			select "(.data.private0|.data.private0.*)";
			select "(.bss.private0|.bss.private0.*)";
		 }
		//}
    }
    ...
    /*	 group PSPR ( run_addr = mem:mpe:pflash0, nocopy, attributes=rx )
		...
		group my_core0_func(run_addr=mem:psram1,ordered )
		 {
			select ".text.private1.*";
		 }
    }
